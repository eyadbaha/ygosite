  ...
  if (children === children.match?.(/({\@[a-z-]+:[\w\s,]+\})/)?.[0]) {
    ...
    if (match) {
      ...
      if (key === "cards") {
        ...
        return (
          <div key={id} className="w-full flex gap-1">
            {cards.map((card) => (
              <div className="w-[10%]">
...
You can import full decks too, like this: {@deck:65f53b2df0f81b41968634f6}
{tabs-split}
Now we are in another tab, let's try getting another card, this time the legendary {@inline-card:4007}.
{tabs-end}`
  );
  return (
    <div className="flex h-[80vh] gap-10">
      <textarea value={text} onChange={(e) => setText(e.target.value)} className="flex-1 text-black p-1 resize-none" />
      <div className="flex-1 border-blue-500 border-2 overflow-y-scroll p-1">

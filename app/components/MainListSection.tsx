      ...
      <ul className="border-[#c77dff] border-2 shadow-purple limit-5">
        {props.list?.map((content, index) => (
          <li className="group" key={index}>
            <Link href={content.link} className="h-14 flex items-center bg-zinc-200 hover:bg-zinc-300 border-b-black border-b-[1px] group-last:border-0">
              <Image src={`/img/icons/${content.tag}.webp`} alt={content.tag} width={64} height={26} quality={75} className="h-auto w-1/6 p-1" />
              <div className="w-4/5">
